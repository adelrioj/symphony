defmodule SymphonyElixir.CoreTest do
  ...
  test "normal worker exit schedules active-state continuation retry" do
    ...
    assert_backoff(due_at_ms, baseline_ms, 1_000)
  end
  ...
  test "abnormal worker exit increments retry attempt progressively" do
    ...
    assert_backoff(due_at_ms, baseline_ms, 40_000)
  end
  ...
  test "first abnormal worker exit waits before retrying" do
    ...
    assert_backoff(due_at_ms, baseline_ms, 10_000)
  end
  ...
  # The orchestrator sets `due_at_ms = System.monotonic_time(:millisecond) + delay` at an instant at
  # or after `baseline_ms`, so `due_at_ms - baseline_ms` is the configured delay plus however long
  # the message took to be handled. That makes the lower bound exact and confines all scheduling
  # jitter to the upper bound. Measuring against the clock *after* the fact instead put the jitter on
  # the lower bound, which is why this raced under `max_cases: 36`.
  # The slack stays far below the 9s gap between adjacent backoff tiers, so a wrong tier still fails.
  @backoff_scheduling_slack_ms 2_000

  defp assert_backoff(due_at_ms, baseline_ms, expected_delay_ms) do
    measured_delay_ms = due_at_ms - baseline_ms

    assert measured_delay_ms >= expected_delay_ms
    assert measured_delay_ms <= expected_delay_ms + @backoff_scheduling_slack_ms
  end
