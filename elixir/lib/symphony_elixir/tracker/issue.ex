defmodule SymphonyElixir.Tracker.Issue do
  ...
  @doc """
  Whether the issue may be dispatched or kept running under a label policy.

  `label_policy` is any map with optional `:required_labels` (every label must
  match) and `:any_labels` (at least one label must match) entries. An empty or
  absent list imposes no constraint. Matching ignores case and surrounding
  whitespace, so a blank configured label matches no issue. A `label_policy`
  that is not a map raises rather than reporting the issue as unroutable.
  """
  ...
  def routable?(%__MODULE__{}, label_policy) when is_map(label_policy), do: false
