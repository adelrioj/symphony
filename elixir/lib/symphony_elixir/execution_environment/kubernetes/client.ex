defmodule SymphonyElixir.ExecutionEnvironment.Kubernetes.Client do
    ...
    with :ok <- credentials(provider),
      ...
      invoke = fn file ->
        ...
        with {:ok, suffix} <- arguments(method, path, file),
             {:ok, %{output: output, status: status}} <- command.(System.find_executable("kubectl") || "kubectl", args ++ suffix, Keyword.merge(opts, timeout_ms: remaining(opts), max_output_bytes: 8_388_608)) do
  ...
  defp pages(config, path, opts, token, acc, seen, restarts) do
    ...
    case request(config, :get, query(path, params), nil, opts) do
      ...
      {:ok, %{status: 410}} when restarts < 2 -> pages(config, path, opts, nil, [], MapSet.new(), restarts + 1)
      {:ok, %{status: code}} when code in [401, 403] -> {:error, {:denied, :kubernetes_inventory}}
      {:error, _} when token != nil and restarts < 2 -> pages(config, path, opts, nil, [], MapSet.new(), restarts + 1)
      {:error, _} = error -> error
      _ -> {:error, {:unknown, :kubernetes_incomplete_inventory}}
    end
  end
