...
- [ ] Write and have Main run a baseline regression through existing Reconcile, using JSON unmarshalling so the old types ignore the proposed fields rather than fail compilation:
...
- [ ] Write real TLS HTTP-server tests that count received POSTs for 429/Retry-After, 503/Retry-After, 307/308 redirects, connection loss after reading the body and success with UID. A redirect target must receive zero requests. Auth configuration/wrappers unsupported by the proof must fail before any send.
...
- [ ] Main adds and runs an existing-API regression before edits:
