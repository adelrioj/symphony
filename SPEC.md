...
- An adapter MAY expose provider-native tools to a selected backend session.
...
- Provider-native tools remain part of the agent toolchain rather than orchestrator business logic.
...
- If provider-native agent tools are implemented:
  ...
  - a standalone MCP bridge preserves the same adapter-bound tool contract when used by a backend
    that does not consume app-server dynamic tools
...
- Agent backend abstraction with a Codex app-server subprocess client using the targeted
...
- A remote host SHOULD provide a reachable shell, writable workspace root, configured coding-agent
  executable, and repository prerequisites. Tracker credentials MAY remain only on the orchestrator
  host.
- For Claude-backed remote runs, each worker SHOULD resolve `claude.command` and either
  `claude.linear_mcp_command` or `symphony` on `PATH`. The orchestrator captures adapter-declared
  tracker secret values into a private MCP server environment config for the session lifetime; the
  Claude process receives those secret names unset and no credential in argv, although a compromised
...
- Remote environment drift:
  - Each host needs the expected shell environment, coding-agent executable, and repository
    prerequisites. Tracker credentials remain in the orchestrator-generated session config rather
