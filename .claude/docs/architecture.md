...
1. **`Orchestrator`** (`lib/symphony_elixir/orchestrator.ex`) — the heart of the system. A single stateful `GenServer` running a polling loop. It holds all live state in its `State` struct (`running`, `claimed`, `completed`, `blocked`, `retry_attempts`, `turn_exhaustions`, `codex_totals`/`codex_rate_limits`), dispatches issues up to `max_concurrent_agents`, and owns retry/reconciliation/cleanup. **This state is concurrency-sensitive** — preserve retry, reconciliation, and cleanup semantics when editing. Blocked entries live in memory only and are cleared on restart. `turn_exhaustions` counts consecutive `agent.max_turns` exhaustions per issue+state; at `agent.max_turn_exhaustions` the issue is parked through `BlockedIssue` instead of getting another continuation retry.
